...
int main()
{
	...
	for (int i = 0; i < NumberOfBlocks * BLOCK_SIZE; i++) {
		printf("%02x ", Message[i]); /*Display message after XOR operation*/
	}
	printf("\n");
...
unsigned char *ReadKey(void)
{
    ...
    fgets(GeneratedKey, 128, file); /*Read 128 bits from file*/
    printf("GeneratedKey :\n");
    for (int i = 0; i < 16; i++) {
        printf("%02x ", GeneratedKey[i]); /*Display the key*/
    }
    printf("\n");
...
void XOR_With_Key(unsigned char *Message, unsigned char *Key, int NumberOfBlocks)
{
	...
	unsigned char BLOCK[16] = {0}; /* Array to store the 128-bit block under consideration*/
	printf("\nXORing with Key:\n");
